...
- **Release content floors now match MikroTik's CLI-Reference migration.** The live manual moved large inline property-reference sections (including OpenVPN, IPsec, and switch QoS) out of prose pages and into `/docs/cli-reference/`, reducing the generic prose-table corpus while the separate CLI-Reference overlay remains populated. `V-db-min-content` keeps blocking degenerate builds with revised generic-table floors; the CLI-Reference discovery, field-count, link-drift, and integrity gates remain unchanged.
