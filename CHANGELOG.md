...
- **`routeros_lookup_property` confidence now grades the row, not the query branch (B-0024 step 4).** Previously `high` meant only "this row is on the page `commands.page_id` links to" and `low` meant "the scoped branch found nothing" — the label described which SQL ran, so every property on a linked page was equally `high` and a correct row found by the global fallback was always `low`. Each row is now scored against the requested menu using the menu paths named in its own documentation section, and results come back best tier first: `high` = the section names that exact menu and the command tree agrees the menu takes the property; `medium` = the section names a neighbouring menu (parent or child), the section names the menu only as a cross-reference while documenting a different one, or only the page matches; `low` = nothing but the property name links the row to the menu. Without `command_path` every row stays `medium` — there is no menu to align to. Measured over the 14,832 (menu, property) pairs the command tree says are real: **31.5% of today's `high` labels survive**, 2,295 demote to `medium`, and **1,940 rows escape a wrongly-`low` label** (835 of them to `high`). This is what makes `/interface/bridge/port` + `pvid` return the two bridge-port sections ahead of the unrelated Apps row (#131), and `/ip/dhcp-server` + `address-pool` rank the DHCP server table above the DHCPv6 one (#58). The returned row set is unchanged.
