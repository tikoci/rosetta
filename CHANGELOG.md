...
- **`hardware-unmatched.tsv` reframed as a classified inventory — new `kind` column.** Every `/hardware` page with no matrix device is now labeled `device` / `accessory` / `module` / `series-or-doc` (66/12/8/16 of the current 102) by a shared, rule-based classifier (`src/hardware-kind.ts`), so the reverse-audit set is a typed inventory rather than an undifferentiated pile: `kind = device` isolates real off-matrix routers worth filing as matrix gaps, while `series-or-doc`/`accessory`/`module` are expected non-devices to skip. Classification carries zero per-slug overrides today, and the logic is shared so a future `include-accessories`-style filter or "include module" cross-link reuses it instead of re-deriving. Regenerated deterministically by `make device-map`; validated by the existing `device-map-check` gate. See `briefings/B-0018-product-naming-three-source-map.md`.
