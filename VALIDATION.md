...
| V-tool-shapes            | Tool response shapes stable for 5 canonical queries                                                                  | `src/mcp-contract.test.ts` Block C in `release.yml` post-extraction    | non-blocking → flip to blocking after one green run | —    |
| V-tool-budget            | Token-budget guardrails on 10 canonical queries                                                                      | `src/mcp-contract.test.ts` Block B in `release.yml` post-extraction    | non-blocking → flip to blocking after one green run | —    |
| V-retrieval-floor        | Recall@10 ≥ baseline − 2pp on 20 hand-curated golden queries; classifier accuracy stable                             | `src/eval/retrieval.ts` in `release.yml`                               | non-blocking → flip to blocking after one green run | —    |
| V-retrieval-self         | Self-supervised eval (~170 auto-generated queries) holds within 5pp of baseline                                      | `src/eval/self-supervised.ts` in `release.yml`                         | non-blocking                                 | —          |
| V-db-min-content         | Built DB has ≥200 pages, ≥1000 commands, ≥100 devices, ≥1000 properties before publish                               | `release.yml` validation step (`scripts/validate-db.ts`)               | blocking                                     | —          |
...
| V-bunx-windows           | Same on Windows — catches v0.8.x EBUSY / readonly-WAL / temp-file class of bugs                                      | new `windows-latest` row in `bunx-smoke` matrix                        | **GAP**                                      | T-0009     |
| V-cross-version-bunx     | Schema-mismatched on-disk DB triggers re-download cleanly                                                            | `src/setup.test.ts`                                                    | blocking                                     | —          |
| V-stdio-handshake        | MCP stdio handshake completes against a real `@modelcontextprotocol/sdk` client; no stdout pollution                 | new `src/mcp-stdio-client.test.ts`                                     | **GAP**                                      | T-0010     |
| V-http-handshake         | MCP HTTP handshake + multi-session lifecycle works end-to-end                                                        | `src/mcp-http.test.ts`                                                 | blocking                                     | —          |
| V-tui-mcp-parity         | Every MCP tool reachable via a `.routeros_*` dot-command in `browse`                                                 | new `src/browse-parity.test.ts`                                        | **GAP**                                      | T-0011     |
| V-cli-flag-uniformity    | CLI flags documented in `MANUAL.md` match `bun src/mcp.ts --help` output                                             | new `src/cli-help.test.ts`                                             | **GAP**                                      | T-0012     |
