...
green the board. Items below are real defects found while authoring the golden set.

**Triage (2026-07-12).** All six items routed to issues: BL-1/2/3 → [#58](https://github.com/tikoci/rosetta/issues/58);
BL-4 → [#59](https://github.com/tikoci/rosetta/issues/59) (`agent-ready`); BL-5 → [#26](https://github.com/tikoci/rosetta/issues/26);
BL-6 → [#60](https://github.com/tikoci/rosetta/issues/60) (low-priority robustness). None fixed inline — the two
new items (#59 classifier precision, #60 cross-version DB) both warrant eval/design care over a drive-by.
...
- **BL-1 — command-path→page→property link broken on core menus (has a durable failing anchor).**
  ...
  `prop-firewall-filter-action` (known-failing). Tracked in [#58](https://github.com/tikoci/rosetta/issues/58).
- **BL-2 — noisy command→page links.** Some `commands.page_id` links are semantically wrong,
  ...
  Same root area as BL-1. Tracked in [#58](https://github.com/tikoci/rosetta/issues/58).
- **BL-3 — dotted WiFi properties not extracted.** `/interface/wifi security.authentication-types`,
  ...
  captured by the Docusaurus property parser. Tracked in [#58](https://github.com/tikoci/rosetta/issues/58) (split out if independent).
- **BL-4 — classifier false-positive on an NL question.** "port forward to an internal server"
  ...
  path-shaped words get mis-detected. Root cause: `detectCommandPath` runs the canonicalizer on
  no-slash input whose first token is a top-level word, greedily pathifying trailing prose.
  Tracked in [#59](https://github.com/tikoci/rosetta/issues/59) (`agent-ready`).
- **BL-5 — see Finding 2** (#26 is not swap-induced). Commented on [#26](https://github.com/tikoci/rosetta/issues/26).
- **BL-6 — `searchAll` crashes on a pre-v8 DB.** `SQLiteError: no such table: device_aliases`
  ...
  is absent. Only bites cross-version DB use (e.g. this audit) — the MCP startup guard
  (`mcp.ts:97`) re-downloads/fails hard on schema mismatch, so the shipped server never reaches it.
  Tracked in [#60](https://github.com/tikoci/rosetta/issues/60) (low-priority robustness).
