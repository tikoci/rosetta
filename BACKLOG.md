...
- Human manually added `hardware_www_matrix.tsv` to repo from manual SQLite .output of `devices_overview` table, but should be regularized in CI.  Additionally the set of "human readable" _generated_ TSV/JSON files should be published on branch for GitHub pages to aid linking (and `main` branch protection rules), as part of CI.  e.g. GH Pages would allow `curl`/etc. on the SQL-based data.  Likely should be broader than "device"-things, so that there are URLs for more table-based results from ETL.
