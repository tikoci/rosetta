...
10. **`browseCommands(arch)` filters enrichment only, not which commands appear.** The arch filter is in the LEFT JOIN ON clause so it controls which `schema_nodes` row enriches each command, but all commands at the path still appear regardless of `_arch`. `diffCommandVersions` uses a NOT IN subquery for true exclusion. Decide: do we want `browseCommands(arch="arm64")` to hide x86-only nodes, or just tag them? If hiding: move arch to a WHERE filter on `schema_nodes._arch` (e.g., `WHERE sn._arch IS NULL OR sn._arch = ?`). Currently the MCP doc says "shows arm64-specific interfaces" but results include all — should be clarified.
11. **`schema_node_presence` has no arch column** — the junction table records (node_id, version) but not which arch(es) the node was present on for that version. For accumulate mode with split arch files, this means we can't answer "did this arm64-only node exist in 7.20?" separately from x86. Adds complexity; defer until item 10 and version retention policy (item 6) are settled.
12. **Accumulate mode silently drops presence for nodes exclusive to non-primary versions** — `schema_nodes` is built from the primary (latest stable) version only. In accumulate mode, a non-primary version's commands that were removed by the primary version can't be FK-referenced in `schema_node_presence`. Those historical presence rows are never inserted. Fix: either expand `schema_nodes` to union all versions' nodes (large), or use a separate `schema_node_presence_legacy` keyed by path+type+version (no FK). Defer until item 6 (GC) is designed.
13. **`commands` UNIQUE(path) is more restrictive than `schema_nodes` UNIQUE(path, type)** — if RouterOS ever has two entries at the same path with different types, `INSERT OR IGNORE` during commands regeneration would silently drop the second row. Not possible today; note for future schema migrations.
