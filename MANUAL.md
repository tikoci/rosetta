...
2. **Database updates (auto on first launch after a package update).** The DB download URL is **pinned to the running package version** — `releases/download/v<VERSION>/ros-help.db.gz` — with `releases/latest/` as a fallback. On every launch, `ensureDbReady()` compares the on-disk DB's stamped `db_meta.release_tag` against the running package's own version — not just its schema version — so a new package version fetches the matching DB on first launch **whether or not that release bumped the schema** (e.g. a content-only bump like `0.11.0-beta.92` → `beta.93`, #76/#23). Dev checkouts (running from a git clone) are exempt from the release-tag comparison — a locally-extracted DB isn't tied to any published release.
