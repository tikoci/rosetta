...
└── tables.tsv          # one row per captured doc table: page/section provenance, shape + size, section deep-link (table_url)
```

`pages.tsv` and `sections.tsv` are the two views of the same core data: `pages.tsv` is one whole-page row (its own word count and `#sections`/`#empty`/table counts), small enough to read directly on GitHub; `sections.tsv` is one row per section, and grouping those rows by `page_id` in a spreadsheet pivots up to (near-)the page totals. The residual is heading-text lines that live in no fragment — `pages.tsv.word_count` is the authoritative whole-page figure, and a per-page sum over `sections.tsv` is the covered-body figure (~98%). `tables.tsv` is the inventory of every captured doc table — one row each with page/section provenance, shape (`column_count`/`data_row_count`/`is_ragged`), source size (`raw_bytes`), and a `table_url` deep-linking to the section that contains it — the "what tables exist and where" list; the table **cell data** is not exported here. Product specs (`products/**`) and per-fragment table cell files (`pages/<slug>/**`) are not yet exported.
