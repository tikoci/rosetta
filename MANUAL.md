...
- **HTML Documentation** — legacy Confluence space export from help.mikrotik.com (March 2026). 317 pages broken into sections, callouts, and property tables (~515K words). MikroTik's current manual is Docusaurus at <https://manual.mikrotik.com>; future doc refreshes require a new extractor rather than another Confluence export.
...
Documentation covers RouterOS **v7 only** and aligns with the long-term release (~7.22) at the March 2026 Confluence-export time. v6 had different syntax and major subsystems — answers for v6 are unreliable. Future official doc updates are expected on <https://manual.mikrotik.com>, whose CLI Reference includes `/console/inspect`-derived command menus and argument types.

## Re-extracting a Local Database

The current ETL is for the legacy Confluence snapshot. It can rebuild the existing corpus, but MikroTik's future documentation updates are on <https://manual.mikrotik.com> and will require a Docusaurus/manual-site extractor plus MCP/TUI result-shape work before rosetta can ingest them.

For the legacy Confluence corpus:
...
Do not assume a fresh `html_url` will exist for future MikroTik docs. See [DESIGN.md](DESIGN.md#docusaurus-manual-migration-future-primary-prose-corpus) and [B-0012](briefings/B-0012-docusaurus-manual-migration.md) for the migration options.
...
```sql
-- Pages (currently from the legacy Confluence HTML export; a Docusaurus
-- extractor will need a revised identity/URL model)
pages (
    id INTEGER PRIMARY KEY,  -- Confluence page ID
    slug, title, path,       -- path = 'RouterOS > Firewall > Filter'
    depth, parent_id,
    url,                     -- legacy help.mikrotik.com/docs/spaces/ROS/pages/{id}/{slug}
    text, code, code_lang,
    author, last_updated,
