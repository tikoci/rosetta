...
- **Parsing:** Every Markdown pipe table is retained generically in normalized `page_tables` → rows → cells storage (raw Markdown remains alongside decoded cells); table-derived properties consume that shared parse and link to the exact source row, while bullet properties stay a separate path. The property classifier remains intentionally narrow (`| Property |` or `| Parameter |`, both seen live) and tolerant of dhcp.md's malformed `check-gateway` emphasis (B-0012 H4). `:::type ... :::` admonitions → `callouts`; non-empty prose before the first real h1–h3 heading becomes a synthetic lead section (`level = 0`, `anchor_id = "_lead"`), followed by the ordinary h1–h3 `sections`. The parser excludes a real source quirk where the raw `.md` repeats the page's H1 title after the AI-generated summary blockquote. Relative Markdown links inside property descriptions are rewritten to live `manual.mikrotik.com` URLs (anchor fragment preserved) instead of left as broken relative paths.
...
- **An entry has a path; a field does not.** The CLI Reference has exactly three entry kinds — Directory, Settings Directory, Command (1,077 entries across 1,070 pages; see "CLI-Reference source reshaped" below). A heading path identifies one entry. Its Argument / Read-only Argument rows are *named fields* of that entry, and its Flag rows are print-output markers. `inspect.json` is a different tree whose `arg` nodes are serialized at coordinates like `/ip/address/add/address` — but that does **not** make the field's path `/ip/address/address`. So `cliref_fields` has a name and an entry FK, never a path. The field→inspect mapping is genuinely many-to-many (for example, `certificate.name` appears under seven commands). Only settable `Argument` rows participate; a same-name inspect input is not evidence that a `Read-only Argument` output field is the same thing.
...
- **Aliasing is auditable, never silent — and scoped.** The manual used to leak internal module segments into headings (`caps-man/acl/access-list` → real `/caps-man/access-list`). The linker resolves these by dropping a single *internal* segment (never the root menu or leaf command — dropping the leaf would false-alias a genuinely-absent submenu onto its parent), stores `match_kind='alias'` with a human-readable `match_detail`, and leaves the source path unchanged. Ambiguous drops stay manual-only rather than guessing. The droppable-segment allowlist is **positional, not global** (#136): a segment is never dropped when the prefix ending at it is itself a published entry, because that makes it a real menu at that point in the path — `interface/ethernet/poe` and `interface/ethernet/switch/qos` are real hardware-gated menus, and dropping them linked PoE and QoS commands to the disjoint SFP/switch-port ones. 967 exact / 0 alias / 110 manual-only on the current corpus. Manual-only entries are first-class: they document commands a CHR `/console/inspect` cannot self-report (build-flag or hardware-gated menus), which is the overlay's whole point.

- **CLI-Reference source reshaped — discovery is a union, and its gap must fail loud (#137).** MikroTik decomposed the module pages (`interface/ethernet.md` with 62 headings) into per-command leaf pages whose slug *is* the CLI path, so a page now carries one entry (1,063 of 1,070; the 7 exceptions publish two `syscap`-gated variants of one path, e.g. `musicswitch` vs `rbswitch`). Two consequences are durable. First, `source_parent_id` is now uniformly `NULL` — it is intra-page heading ancestry, and a one-heading page has none; `toc_group` (URL parent) is what remains for grouping. Second, and the reason this was a bug rather than a refresh: **the sitemap serves a branching menu as a trailing-slash category URL with no `.md` of its own**, while the menu's own Directory entry is published at `<dir>/<basename(dir)>.md` (`app/` → `app/app.md`) and appears only in `llms.txt`. Sitemap-only discovery therefore dropped 256 of 1,070 leaves — ~24% of the corpus, and the field-heavy quarter (fixing it moved fields 5,944 → 10,938). Discovery is now the union of both inventories, and `V-cliref-discovery` asserts every category dir contributes its leaf, so the next inventory change fails the build instead of quietly shrinking the corpus. The earlier "generated category stubs — every `.md` variant 404s" note was verified against the old corpus with the wrong candidate URLs; a 404 on a *probe* is evidence about the probe, not about the page.
